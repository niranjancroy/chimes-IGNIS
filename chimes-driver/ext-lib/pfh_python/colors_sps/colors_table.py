def colors_table( age_in_Gyr, metallicity_in_solar_units, 
    ...
    if (CHABRIER_IMF==1): fname=froot+'colors.chabrier.hdf5'
    if (SALPETER_IMF==1): fname=froot+'colors.salpeter.hdf5'

    ## old format - deprecated as its very machine-and-version-dependent
    #lut = open(fname,'r');
    #lut_dat = lut.read();
    ...
    lut=h5py.File(fname,'r')
    Nl=lut['Number_of_Bands'][()]
    Na=lut['Number_of_Ages'][()]
    Nz=lut['Number_of_Metallicities'][()]
    age_grid=np.array(lut['Age_Grid_LogAgeInYr']); z_grid=np.array(lut['Metallicity_Grid_LogMetalMassFraction']);
    l_all=np.array(lut['Luminosity_LookUpTable_LMinSolar']);
