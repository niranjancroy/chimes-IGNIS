...
import os.path
import visualization.image_maker_box as imaker
import gadget
import sys

def make_box_movie(
    xmax_of_box=200., #"""scale (in code units) of movie box size"""
    snapdir='/home/cchayward/scratch/VoT/hires_oldwfb',
    outputdir_master='/home/cchayward/scratch/VoT/hires_oldwfb/movie_frames',
    cosmological=1,
    show_gasstarxray='gas', #"""what to plot; options are 
    pixels=1080,
    frames_per_gyr=100.,
    time_min=0.1724139995256639,
    time_max=0.2747249995724909,
    frame_min=0,
    frame_max=1.0e10,
    i_snap_min=74,
    i_snap_max=158,
    ...
    temp_max=1.0e6, #"""max gas temp to show"""
    temp_min=3.0e2, #"""min gas temp to show"""
    theta_0=90., #"""initial theta value for camera"""
    delta_theta=0., #"""amount to rotate in theta direction"""
    phi_0=120., #"""initial phi value for camera"""
    delta_phi=0., #"""amount to rotate in phi direction"""
    center_on_bh=0, #"""if have a bh particle, uses it for the centering"""
    center_on_com=1, #"""simple centering on center of mass"""
    ...
    nasa_colors=0, #"""use nasa color-scheme for three-color images"""
    ...
    do_with_colors=0, #"""make color images"""
    threecolor=0, #"""do or dont use the standard three-color projection for images"""
    camera_opening_angle=60., #"""camera opening angle (specifies camera distance)"""
    ...
    snapshot_name='snap_z10n192_B1hr', #"""specify snapshot prefix (neglect last '_') if you use something besides 'snapshot'"""
    zthick_0=1000.
    ):
    ...
    print(time_frame_grid)

#    x_plot_scale_grid, y_plot_scale_grid, phi_frame_grid, theta_frame_grid, coa_frame_grid, \
#      maxden_grid, dynrange_grid, zthick_grid = return_camera_parameters(time_frame_grid,
#        xmax_of_box=xmax_of_box,ymax_of_box=xmax_of_box,
#        phi_0=phi_0, delta_phi=delta_phi, theta_0=theta_0,
#        delta_theta=delta_theta, coa_0=camera_opening_angle,
#        delta_camera_distance=delta_camera_distance, maxden_0=maxden_0, delta_maxden=0.,
#        dynrange_0=dynrange_0, delta_dynrange=0., zthick_0=zthick_0, delta_zthick=delta_zthick,
#        frames_per_gyr=frames_per_gyr)

    t=(np.array(range(time_frame_grid.size),dtype='f'))/time_frame_grid.size
    x_plot_scale_grid = t*0.0 + xmax_of_box # CCH: should be xmax_of_box - snapnum*3.6
    #x_plot_scale_grid = 533.6 - t*(533.6-231.2)
    y_plot_scale_grid = x_plot_scale_grid
    phi_frame_grid = phi_0 + 0.0*t
    theta_frame_grid = t*0.0 + theta_0
    coa_frame_grid = camera_opening_angle + 0.0*t
    dynrange_0 = 1.0e4
    maxden_0 = 1.e-5
    maxden_grid = t*0.0 + maxden_0
    dynrange_grid=dynrange_0 + 0.*t
    #zthick_0 = 1500.
    #zthick_grid = zthick_0 +t*0.0 # CCH: should be zthick_0 - <snap num> * 4.25
    #zthick_grid = 1185.5 - t*(1185.5-828.5)
    zthick_grid = zthick_0 + 0.*t # set thickness of z slice to be 1 *physical* Mpc
    print('zthick_grid = ',zthick_grid)
    ...
    for i_snap in snap_grid_to_loop:
        ...
        if(n_frames_to_do>0):
            ...
            xmax_tmp = np.max(x_plot_scale_grid)*1.5 + 10.
            ...
            id_i,m_i,x_i,y_i,z_i,vx_i,vy_i,vz_i,h_i,c_i,zm_i = \
                load_pos_vel_etc(snapdir, snapshot_list[i_snap],\
                    ...
                    min_stellar_age=min_stellar_age,snapshot_name=snapshot_name);
            if (SHOW_STARS==1):
            ...
            id_f,m_f,x_f,y_f,z_f,vx_f,vy_f,vz_f,h_f,c_f,zm_f = \
                load_pos_vel_etc(snapdir, snapshot_list[i_snap+1], \
                    h0=use_h0,four_char=four_char,cosmological=cosmological,skip_bh=skip_bh, \
                    GAS=SHOW_GAS,BOX_MAX=xmax_tmp,CENTER_BOX=center_f,snapshot_name=snapshot_name);
            if (SHOW_STARS==1):
            ...
            for i_frame in range(n_frames_to_do):
                ...
                ## make particles outside z slice have zero mass
                zthick_tmp = zthick_grid[j_of_frame]*a # assumes that thickness is comoving
                print("z range before slicing: ",np.min(z_all)," - ",np.max(z_all))
                print("thickness of slice (physical kpc): ",zthick_tmp)
                print("Total mass before slicing: ",m_all.sum())
                m_all[np.abs(z_all) > zthick_tmp] = 0.0
                print("Total mass after slicing: ",m_all.sum())
                ...
                image24, massmap = \
                  imaker.image_maker( snapdir_specific, j_of_frame, \
                    ...
                    project_to_camera=1, camera_opening_angle=coa, \
                    ...
                    input_data_is_sent_directly=1, include_lighting=0., \
...
def load_pos_vel_etc( snapdir, snapnum, \
    ...
    for ptype in ptypes:
        ppp=gadget.readsnap(snapdir,snapnum,ptype,h0=h0,cosmological=cosmological,skip_bh=skip_bh,
            snapshot_name=snapshot_name);
        ppp_head=gadget.readsnap(snapdir,snapnum,ptype,header_only=1,h0=h0,
            cosmological=cosmological,skip_bh=skip_bh,snapshot_name=snapshot_name);
        if(ppp['k']==1):
            n=ppp['m'].size;
            if(n>1):
                m=ppp['m']; id=ppp['id'];
                ...
                if (ptype==2): ## need to assign ages and metallicities
                    cc = np.maximum(min_stellar_age , np.zeros(n)+2.0+ppp_head['time'])
                    zm = np.zeros(n) + 0.01;
                if (ptype==3): ## need to assign ages and metallicities
                    cc = np.maximum(min_stellar_age , np.zeros(n)+6.0+ppp_head['time'])
                    zm = np.zeros(n) + 0.002;
...
def build_camera_centering( snums, snapdir ,
    ...
    use_h0=1,four_char=0,cosmological=0,skip_bh=0,snapshot_name='snapshot'):
        ...
        if (center_on_com==1):
            ...
            if (PPP['k']==1):
                if (PPP['m'].size > 0):
                    ...
                    for j in [0,1,2]: cen[i,j]=np.sum(p[:,j]*wt)
            continue
