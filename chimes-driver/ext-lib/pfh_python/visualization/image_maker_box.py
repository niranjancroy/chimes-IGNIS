...
def test2():
    smaster='/Users/phopkins/Downloads/gal_lookie/'
    
    #snums=[300,400]
    snums=[300] # 300,50-100
    sdirs=['m12i_ref11']
    #snums=[500,600] # 500,20-50
    #sdirs=['m12i_ref11']#,'m12i_ref11_tsph']
    for sdir in sdirs:
        for snum in snums:
            #for x00 in [20,50,100,200]:
            for x00 in [300]:
                ext='_len'+str(x00)
                dynr = 1.e3 * (0.01*x00)
                out0,out1 = image_maker(sdir,snum,add_extension=ext,
                    snapdir_master=smaster,outdir_master='./',pixels=720,
                    do_with_colors=1,cosmo=1,xrange=np.array([-1.,1.])*x00,yrange=np.array([-1.,1.])*x00,
                    #dynrange=1.e3,maxden=6.e-3, show_gasstarxray='gas',include_lighting=1,
                    dynrange=dynr,maxden=6.e-3, show_gasstarxray='gas',include_lighting=1,
                    theta=90.,phi=0.,threecolor=1, project_to_camera=0) # edgeon
