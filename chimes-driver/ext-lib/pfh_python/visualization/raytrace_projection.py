...
def gas_raytrace_temperature( TEMPERATURE_CUTS, \
        ...
        isosurfaces=0 , add_temperature_weights=0, openmp=True, hmax_scaling=100. ):
    ...
    return raytrace_projection_compute(gas_x,gas_y,gas_z,gas_hsml,gas_mass,\
        ...
        openmp=openmp, hmax_scaling=hmax_scaling);
...
def stellar_raytrace( BAND_IDS, \
    ...
    for i_band in range(Nbands):
        nu_eff = util.colors_table(np.array([1.0]),np.array([1.0]), \
            BAND_ID=BAND_IDS[i_band],RETURN_NU_EFF=1, QUIET=1);
        ...
        l_m_ssp = util.colors_table( stellar_age, stellar_metallicity/0.02, \
            ...
            UNITS_SOLAR_IN_BAND=1, QUIET=1); ## this is such that solar-type colors appear white
    ...
    #kappa *= np.array([0.8,0.6,0.205])*0.75 * 0.5
    kappa *= np.array([1.0,0.75,0.5])
    kappa *= KAPPA_UNITS;
    ...
    return raytrace_projection_compute(np.r_[stellar_x, gas_x], np.r_[stellar_y, gas_y], np.r_[stellar_z, gas_z],
                                       np.r_[stellar_hsml, gas_hsml], np.r_[stellar_mass_attenuation, gas_mass_metal],
                                       np.r_[lums[0,:],gas_lum], np.r_[lums[1,:],gas_lum], np.r_[lums[2,:],gas_lum],
...
def raytrace_projection_compute( x, y, z, hsml, mass, wt1, wt2, wt3, \
    ...
    TRIM_PARTICLES=1, openmp=True, hmax_scaling=100. ):
    ...
    if openmp:
        import os
        if isinstance(openmp, bool) or openmp==1:
            if 'OMP_NUM_THREADS' not in os.environ:
                os.environ['OMP_NUM_THREADS'] = '8'
                print("Setting OMP_NUM_THREADS to default value of {}".format(os.environ['OMP_NUM_THREADS']))
            else:
                print("Using environment setting of {} threads".format(os.environ['OMP_NUM_THREADS']))
        else:  #has to be a number > 1, so that means i use it
            os.environ['OMP_NUM_THREADS'] = str(openmp)
            print("Setting OMP_NUM_THREADS to {}".format(os.environ['OMP_NUM_THREADS']))
    else:
        print("Will call serial version of raytrace")
    ...
    if openmp:
        ...
        exec_call=util.return_python_routines_cdir()+'/RayTrace_RGB_Serial/raytrace_rgb.so'
    # exec_call = util.return_python_routines_cdir() + '/RayTrace_RGB/raytrace_rgb.so'
