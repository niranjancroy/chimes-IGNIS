...
def construct_spin_movie_single_sim(
    ...
    if(cen[0]==0.):
        cen=gadget.calculate_zoom_center(smaster+sdir,snum)
        print('CALCULATED CENTER == ',cen)
    
    P=gadget.readsnap(smaster+sdir,snum,0,cosmological=1)
    ...
    z_vec = np.dot(return_rotation_matrix(theta),z_vec)
    x_vec,y_vec = util.return_perp_vectors(z_vec)
    y_vec_toplot = y_vec; z_vec_toplot = z_vec;
    ...
    for i in i_loop_list:
        ...
        out0,out1 = im_make.image_maker(sdir,snum,center=cen,
            ...
            projection_vector_x=x_vec,projection_vector_y=y_vec_toplot,projection_vector_z=z_vec_toplot);        
