...
def contour_makepic( x, y, z, hsml, weight, 
    ...
    if (set_percent_maxden !=0) or (set_percent_minden !=0):
        ...
        Msort=Msort[(Msort>0.)&(np.isfinite(Msort))];
        if (set_percent_maxden != 0): ma=Msort[np.round(set_percent_maxden*Msort.size).astype(int)];
        mi=ma/set_dynrng;
        if (set_percent_minden != 0): mi=Msort[np.round(set_percent_minden*Msort.size).astype(int)];
        if (set_percent_maxden == 0): ma=mi*set_dynrng;
        ok=(Msort > 0.) & (np.isfinite(Msort))
...
def simple_makepic( x, y, 
    ...
    bad = (MassMap_1 <= 0.) | (np.isnan(MassMap_1)) | (Pic_1 <= 1.0)
    M1Min=np.min(MassMap_1[MassMap_1 > 0.]); MassMap_1[bad]=M1Min;

    if (color_temperature==1):
        if(invert_colorscale==0):
            Pic_1[bad]=np.max(Pic_1);

        bad2 = bad | (np.isnan(MassMap_2)) | (MassMap_2 <= 0.)
        tempMap = MassMap_2/MassMap_1
        M2Min=np.min(MassMap_2[MassMap_2 > 0.]); 
        #tempMap[bad2] = (MassMap_2[bad2] + M2Min) / M1Min;
        tempMap[bad2] = (M2Min) / M1Min;
        if(invert_colorscale==0):
            tempMap[bad2] = (np.min(MassMap_2[MassMap_2 > 0.])) / np.max(MassMap_1[MassMap_1 > 0.]);
        print('MinMax: ',np.min(Pic_1),np.max(Pic_1),np.min(MassMap_2),np.max(MassMap_2),np.min(tempMap),np.max(tempMap))
