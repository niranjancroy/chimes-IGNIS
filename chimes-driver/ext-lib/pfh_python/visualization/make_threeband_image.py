...
def layer_extra_image_onto_threeband(im_3band, map_3band, im_layer, map_layer):
...
def make_threeband_image_process_bandmaps(r,g,b, \
    dont_make_image=0, maxden=0, dynrange=0, \
    color_scheme_nasa=1, color_scheme_sdss=0 , \
    filterset = ['r','g','b'] , set_f_saturated=0.0001, **kwargs):
    ...
    f_saturated=set_f_saturated  ## fraction of pixels that should be saturated
    if(set_f_saturated > 0):
        x0=int_round( f_saturated * (np.float(checklen(r)) - 1.) ); 
        for rgb_v in [r,g,b]: 
            rgbm=single_vec_sorted(rgb_v,reverse=True); 
        ...
        if (maxrgb > 1.): 
            ...
            if(color_scheme_nasa==1): maxnorm = minnorm * ((maxnorm/minnorm)**maxrgb)
    ...
    return image24, cmap_m, maxnorm, minnorm; ## return both processed image and massmap
...
def make_threeband_image( x, y, lums, hsml=0, xrange=0, yrange=0, \
    ...
    g=r_band_map #*5.7
    b=u_band_map #*7.8
    image24,cmap_m, image_max, image_min = make_threeband_image_process_bandmaps(r,g,b, \
