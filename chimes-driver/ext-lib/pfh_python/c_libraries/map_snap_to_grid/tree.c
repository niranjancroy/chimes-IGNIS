...
void carry_ray_across_cell(Ray_struct *ray, int cell_id)
{
	...
	ray->nh[i_vel]				+= rhodr;
	ray->nh_hot[i_vel] 			+= CELL[cell_id].U.rho_hot * rhodr;
	ray->Z[i_vel]				+= CELL[cell_id].U.Z * rhodr;
	ray->neutral_frac[i_vel] 	+= CELL[cell_id].U.neutral_frac * rhodr;
	}
	...
	ray->nh[i_vel] += CELL[cell_id].U.rho * (1./beta_rho) * (exp(beta_rho*dr) - 1.);
	...
	beta = beta_rho;	/* don't change */
		...
		ray->nh_hot[i_vel]			+= CELL[cell_id].U.rho_hot * beta; /* plug the value in here */
		}
	beta = beta_rho;
		...
		ray->Z[i_vel]				+= CELL[cell_id].U.Z * beta;
		}
	beta = beta_rho;
		...
		if (fabs(beta) < 1.0e-10) beta = 1.0e-10 / dr;
		beta = (1./beta) * (exp(beta*dr)-1.) * CELL[cell_id].U.rho;
		ray->neutral_frac[i_vel]	+= CELL[cell_id].U.neutral_frac * beta;
		}
	}
...
void integrate_ray_to_escape(Ray_struct *ray, int origin_cell_id)
{
	...
	int i_vel;
	for (i_vel=0;i_vel<VELOCITY_GRID_NUMBER;i_vel++) {
	ray->nh[i_vel]			  = 1.0e-28;
	ray->nh_hot[i_vel]		  = 1.0e-28;
	ray->Z[i_vel]			  = 1.0e-28;
	ray->neutral_frac[i_vel]  = 0.0;
	}
