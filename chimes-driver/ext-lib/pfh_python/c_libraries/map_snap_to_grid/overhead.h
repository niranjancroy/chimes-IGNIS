...
#define VELOCITY_GRID_RANGE 	  5000.0  /* maximum absolute value of velocity in km/s of gridding in velocity space */
#define VELOCITY_GRID_RESOLUTION    50.0  /* bin resolution in km/s for gridding in velocity space */
#define VELOCITY_GRID_NUMBER      ((int )(2.0*VELOCITY_GRID_RANGE/VELOCITY_GRID_RESOLUTION)) /* number of points in the velocity grid */
...
#define TREE_FRAC_HSML        1.0   /* Fraction of h_sml for minimum size of tree cells */
#define FRACTIONAL_STEPSIZE   1.0	/* Fraction of h_sml to step out when using brute-force integration */
...
#define USE_FULL_NEIGHBOR_CALC YES	/* If YES (not equal 0), then use a direct nearest neighbor calculation for 
...
#define USE_CELL_GRADIENTS	NO		/* option to calculate the effects of a gradient in a given quantity across the cells -- 
...
typedef struct Ray_s{
  ...
  float nh[VELOCITY_GRID_NUMBER];				/* integrated gas column density along the ray, in each of several velocity bins */
  float nh_hot[VELOCITY_GRID_NUMBER];			/* integrated gas column density of the hot phase of the ISM along the ray */
  float Z[VELOCITY_GRID_NUMBER];				/* integrated mass-weighted metallicity along the ray */
  float neutral_frac[VELOCITY_GRID_NUMBER];		/* integrated mass-weighted neutral fraction along the ray */
  float Temp[VELOCITY_GRID_NUMBER];             /* integrated mass-weighted temperature along the ray */
