...
/* Fundamentally, this code is identical to the basic LOS column density code. 
	the difference is that it is designed to calculate for an array of background 
	sources, rather than particles within the simulation */
...
int getnh(int argc, void *argv[]) 
{
  ...
  fprintf(stderr, "Ngas = %d Nstar = %d Nbh = %d Ntotal = %ld\n", Ngas, Nstar, Nbh, All.N_total);
...
*/
  ...
  printf("NUMBER_OF_RAYS = %d, N_rays = %d, sizeof(Ray_struct) = %lu\n", NUMBER_OF_RAYS, N_rays, sizeof(Ray_struct) ); 
  ...
  if(!(ray = malloc(N_rays*sizeof(struct Ray_s))))
     {
        printf("failed to allocate memory: N_rays= %d  size= %lu\n",N_rays,N_rays*sizeof(struct Ray_s));
        exit(0);
     }
 ...
 */
  ...
  for (ray_num=0; ray_num<NUMBER_OF_RAYS; ray_num++) { 
	...
	for (i_vel=0;i_vel<VELOCITY_GRID_NUMBER;i_vel++) {
	    ray[ray_num].Z[i_vel] /= ray[ray_num].nh[i_vel];
	    ray[ray_num].neutral_frac[i_vel] /= ray[ray_num].nh[i_vel];
	    ray[ray_num].nh[i_vel]   *= nh_prefactor;
	    ray[ray_num].nh_hot[i_vel] *= nh_prefactor;

    // if (DEBUG) 
    if (!(ray_num%100)&&!(i_vel%20)) fprintf(stderr, "ray_num %d origin_cell %d origin_type %d origin_id %d theta %.1f phi %.1f nh %.3f Z %f\n",ray_num, RAY_ORIGIN_CELL, ray[ray_num].ORIGIN_TYPE, ray[ray_num].ORIGIN_ID,ray[ray_num].theta,ray[ray_num].phi,log10(ray[ray_num].nh_hot[i_vel]),ray[ray_num].Z[i_vel]);


    /* write the results to the input arrays */ 

    on = OUT_NH + ray_num*VELOCITY_GRID_NUMBER + i_vel; 
    *on = ray[ray_num].nh_hot[i_vel]; 
    //printf("  ? nh = %e \n",ray[ray_num].nh_hot);

    oz = OUT_Z + ray_num*VELOCITY_GRID_NUMBER + i_vel;
    *oz = ray[ray_num].Z[i_vel];
