...
#define  PI        3.1415927
    void ngb3d_treesearch(struct NODE *this, float xyz[3], float sr2);
    ...
    do
    {
        ...
        numngb=0;
        ngb3d_treesearch(nodes, xyz, sr2);  rep++;
        
        if(rep > 300)
        {
            printf("...long iteration: numngb = %d sr = %g hmax = %g hmin = %g desnum = %d desnumtol = %d rep = %d xyz=%g/%g/%g left=%g right=%g\n",
                   numngb,sr,hmax,hmin,desngb,desngb_tolerance,rep,xyz[0],xyz[1],xyz[2],left,right);
        }
            ...
            if(numngb < desngb - desngb_tolerance)
            {
                ...
                if(sr<hmax)
                {
                    if(numngb>1)
                    {
                        ...
                        fac = pow(((float)desngb)/((float)numngb),1.0/3.0);
                        double flim = 1.05 + 0.4 / pow((1.+(float)rep),0.33);
                        if(fac > flim) fac=flim;
                    }
                    else
                    {
                        fac = 1. + 0.5 + 1.0 / pow((1.+(float)rep),0.33);
                    }
                    double sr_prev = sr;
                    sr *= fac;
                    if(sr <= left) 
                    {
                        //sr = left * (1. + 0.01 + 0.05/pow((1.+(float)rep),0.25));
                        double rhs = right; 
                        if(sr_prev < rhs) rhs = sr_prev;
                        double lhs = left;// * (1. + 0.05/pow((1.+(float)rep),0.25));
                        //if(lhs > sr_prev) lhs = sr_prev;
                        sr = 0.5*(lhs + rhs);
                    }
                    if(sr >= right) 
                    {
                        //sr = right / (1. + 0.01 + 0.05/pow((1.+(float)rep),0.25));
                        double lhs = left; 
                        if(sr_prev > lhs) lhs = sr_prev;
                        double rhs = right;// / (1. + 0.05/pow((1.+(float)rep),0.25));
                        //if(rhs < sr_prev) rhs = sr_prev;
                        sr = 0.5*(lhs + rhs);
                    }
                    if(sr>hmax) sr=hmax;
                    if(sr<hmin) sr=hmin;
            ...
            if(numngb > desngb + desngb_tolerance)
            {
                ...
                if(sr>hmin)
                {
                    if(numngb>1)
                    {
                        ...
                        double flim = 0.95 - 0.3 / pow((1.+(float)rep),0.33);
                        if(fac < flim) fac=flim;
                    }
                    else
                    {
                        fac = 1. / (1. + 0.5 + 1.0 / pow((1.+(float)rep),0.33));
                    }
                    double sr_prev = sr;
                    sr *= fac;
                    if(sr <= left) 
