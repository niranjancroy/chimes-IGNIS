...
{ 
  ...
  float  xmin[3],xmax[3];
  long    count;          /* total # of particles in cell      */
  struct NODE *father,*suns[8];
  long    first;          /* index of first particle           */
} *nodes;


static long    numnodes,MaxNodes;
...
static long    *next,*previous;   /* Link-List for particle groups */

static long    *ngblist,numngb;
static float  *r2list;
...
float ngb3d_treefind(float xyz[3], long desngb, float hguess,long **ngblistback, float **r2listback,
		     float hmax, long *ngbfound)
{
#define SECFACTOR  1.2
#define  PI               3.1415927
  void ngb3d_treesearch(struct NODE *this);

  float selectb(unsigned long k, unsigned long n, float arr[],long ind[]);

  float sr,sr2,h2max;  /* search radius */

  long  i,ind,ni,j,subnode,fak,k,rep=0;
  float dx,dy,dz,r2;
...
void ngb3d_treeallocate(long npart,long maxnodes)  /* usually maxnodes=2*npart is suffiecient */
{
  ...
  if(!(nodes=malloc(MaxNodes*sizeof(struct NODE))))
    {
      fprintf(stderr,"Failed to allocate %ld nodes.\n",MaxNodes);
      exit(0);
    }
  
  if(!(next=malloc(N*sizeof(long))))
    {
      fprintf(stderr,"Failed to allocate %ld spaces for next array\n",N);
      exit(0);
    }

  if(!(previous=malloc(N*sizeof(long))))
    {
      fprintf(stderr,"Failed to allocate %ld spaces for previous array\n",N);
      exit(0);
    }

  if(!(ngblist=malloc(N*sizeof(long))))
    {
      fprintf(stderr,"Failed to allocate %ld spaces for ngblist array\n",N);
      exit(0);
    }

  if(!(r2list=malloc(N*sizeof(float))))
    {
      fprintf(stderr,"Failed to allocate %ld spaces for r2list array\n",N);
      exit(0);
    }
}
...
{
  long i,j,k,subp,subi,p,ni,subnode,fak;
...
*/
      ...
      if(th->count==2)  /* cell was occcupied with one particle */
	{
	  while(1)
		...
		{
		  fprintf(stderr,"maximum node number %ld in neighbour tree reached.\n",numnodes);
		  fprintf(stderr,"subi= %ld  subp= %ld\n",subi,subp);
		  fprintf(stderr,"i=%ld  xyz= %g  %g  %g\n",i,Pos[i]->xyz[0],Pos[i]->xyz[1],Pos[i]->xyz[2]);
		  fprintf(stderr,"p=%ld  xyz= %g  %g  %g\n",p,Pos[p]->xyz[0],Pos[p]->xyz[1],Pos[p]->xyz[2]);
		...
		if(Pos[i]->xyz[j]>th->center[j])
	      ...
	      if(th->len < 1.0e-5)
		{
		    subi = (long) (4.0 * rand() / (RAND_MAX + 1.0));
		}
  ...
  printf("Tree contruction finished. Number of nodes: %ld\n",numnodes);
}
