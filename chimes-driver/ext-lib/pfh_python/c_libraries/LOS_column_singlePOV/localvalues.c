...
{
  long i, flag, stars_spawned, tot_spawned, stars_converted, tot_converted;
   ...
   */
   ...
   long DO_EFFECTIVE_EOS_FLAG=0;

   if(DO_EFFECTIVE_EOS_FLAG==0) {
 
                f = 1;
                //temperature_hot         = convert_u_to_temp(u_tot, rho_tot, &ne_temp);
                yhelium = (1 - HYDROGEN_MASSFRAC) / (4 * HYDROGEN_MASSFRAC);
                mu = (1 + 4 * yhelium) / (1 + yhelium+ne_given);
                mean_weight= mu*PROTONMASS;
                temperature_hot= mean_weight/BOLTZMANN * GAMMA_MINUS1 * u_tot *
