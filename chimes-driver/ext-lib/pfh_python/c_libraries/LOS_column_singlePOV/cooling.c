...
#define TABLESIZE 171		/* Max # of lines in TREECOOL */
...
static long nheattab;		/* length of table */


void ReadIonizeParams(char *fname)
{
  // for the sake of portability, we're not using the actual treecool file, but 
  //   a locally fixed matrix here (prevents crashes on not finding)
  long i;
  //FILE *fdcool;
...
int n_rows_treecool=TABLESIZE;
int n_columns_treecool=7;
double treecool_file[1197] = {
...
0.850, 0.00000e+00, 0.00000e+00, 0.00000e+00, 0.00000e+00, 0.00000e+00, 0.00000e+00 };
  ...
  for(i = 0; i < TABLESIZE; i++)
  {
    inlogz[i] = treecool_file[i*n_columns_treecool + 0];
    gH0[i] = treecool_file[i*n_columns_treecool + 1];
    gHe[i] = treecool_file[i*n_columns_treecool + 2];
    gHep[i] = treecool_file[i*n_columns_treecool + 3];
    eH0[i] = treecool_file[i*n_columns_treecool + 4];
    eHe[i] = treecool_file[i*n_columns_treecool + 5];
    eHep[i] = treecool_file[i*n_columns_treecool + 6];
    /*
  ...
  if(DEBUG)
    printf("\n\nread ionization table with %ld entries in file `%s'.\n\n", nheattab, fname);
}
