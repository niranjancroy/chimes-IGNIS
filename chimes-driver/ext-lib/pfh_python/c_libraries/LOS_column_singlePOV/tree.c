...
void free_the_tree(void) { 
  ...
  //MEM must make sure this is the same as the value set in "initialize_the_tree" below
  ncells = All.N_gas*(long)(4/TREE_FRAC_HSML); 
  /*
  for (i=0; i < ncells; i++) { 
    if (CELL[i].N_particles_contained > 0) free(CELL[i].particle_IDs); 
    if (CELL[i].sub_cell_check != 1) free(CELL[i].dU);
  }
  */

  for (i=ncells-1; i >= 0; i--) {
    if (CELL[i].HAS_dU_ALLOCATED==1) free(CELL[i].dU); //MEM
    //MEM free(CELL[i].dU);
    //MEM if (CELL[i].sub_cell_check != 1) free(CELL[i].dU);
    //MEM now these are always allocated: slightly higher memory cost but much greater stability
  }
  printf(".. tree freed dU \n");fflush(stdout);
  for (i=ncells-1; i >= 0; i--) {
    //MEM if(CELL[i].N_particles_contained > 0) free(CELL[i].particle_IDs);
    if (CELL[i].HAS_pIDs_ALLOCATED==1) free(CELL[i].particle_IDs); //MEM
  }
  printf(".. tree freed particle_IDs \n");fflush(stdout);
  free(CELL);
 ...
 */
 void initialize_the_tree(void)
 {
    ...
    printf("MAXDIST == %g  MINSIZE == %g \n",MAX_DISTANCE_FROM0,TREE_MIN_SIZE);

	extern struct CELL_STRUCT *CELL;
       	
	//np = All.N_gas*(long)(4/TREE_FRAC_HSML);
        np = All.N_gas*(long)(4/TREE_FRAC_HSML);
	if(!(CELL=malloc(np*sizeof(struct CELL_STRUCT))))
	  {
	    fprintf(stderr,"failed to allocate memory (tree C).\n");
	    exit(0);
	  }
	for(j=0;j<np;j++) {
	  CELL[j].N_particles_contained = 0;
	  CELL[j].sub_cell_check = -1;
//MEM :: this costs more memory but prevents seg-faults on freeing the memory later ::
//MEM      CELL[j].dU = (LOCALVAL *)calloc(3,sizeof(LOCALVAL));
//MEM :: lets try including the explicit variables checking if the 'optional' pointers are allocated
      CELL[j].HAS_dU_ALLOCATED = 0; //MEM
      CELL[j].HAS_pIDs_ALLOCATED = 0; //MEM
	}
	printf("Number of cells allocated: %ld\n", np);

	long CURRENT_CELL_ID = 0;
	ALL_CELL_COUNTER = 1;
	...
	for (n_cell=ALL_CELL_COUNTER; n_cell<ALL_CELL_COUNTER+N_SUBCELL_3; n_cell++) {
		...
		//long *testmalloc;
		//testmalloc = (long *)malloc(3*sizeof(long));
...
//	for (n_cell=n_subshell_i; n_cell<n_subshell_f; n_cell++) build_cell(n_cell);
	...
	printf("Tree constructed, %ld total cells \n",ALL_CELL_COUNTER);
...
//printf("t2 \n");fflush(stdout);
    ...
    float Hmax= 10000.0;
      h=CELL[cell_ID].width;    
        /* make this adaptive b/c otherwise too large a guess takes a LONG time to converge */
      Hmax=5.*h;
//printf("t3 \n");fflush(stdout);
...
/*
    long particle_id = 0;
    ngb_list= (long *)malloc(Num_neighbors*sizeof(long));
...
//MEM    	
        CELL[cell_ID].dU = (LOCALVAL *)calloc(3,sizeof(LOCALVAL)); //MEM
        CELL[cell_ID].HAS_dU_ALLOCATED = 1;
	for (j=0; j<3; j++) {	
...
long find_cell_from_scratch(float pos[3])
{
	...
	long j, n_x[3], n_sub_ID=0, cell_ID=0;

	cell_ID = 0;
	while (CELL[cell_ID].sub_cell_check == 1) {
		for (j=0; j<3; j++) {
			n_x[j] = (long)(((double)pos[j] - (double)CELL[cell_ID].min_x[j])/((double)CELL[cell_ID].width/((double)N_SUBCELL)));
			if(n_x[j]<0) n_x[j]=0;
			if(n_x[j]>1) n_x[j]=1;
		}
		n_sub_ID = n_x[0]*N_SUBCELL_2 + n_x[1]*N_SUBCELL + n_x[2];
...
long find_cell_from_nearby_cell(float cell_ID_0, float pos[3])
{
	...
	long j, n_x[3], n_sub_ID, cell_ID, in_cell_check;
	...
	while (CELL[cell_ID].sub_cell_check == 1) {
		for (j=0; j<3; j++) {
			n_x[j] = (long)((pos[j] - CELL[cell_ID].min_x[j])/(CELL[cell_ID].width/N_SUBCELL));
			if (n_x[j] < 0) n_x[j] = 0;
...
void carry_ray_across_cell(Ray_struct *ray, long cell_id)
{
	...
	beta = beta_rho;	/* don't change */
		...
		if (dr > 0.) {
		if (fabs(beta) < 1.0e-10) beta = 1.0e-10 / dr;
	...
	//printf("%ld .. ",CELL[cell_id].N_particles_contained);
	return;
}
...
void integrate_ray_to_escape(Ray_struct *ray, long origin_cell_id)
{
	...
	long counter = 0;
	long cell_id = origin_cell_id;
	while (cell_id >= 0 && counter <= 1000000) {
