...
void readsnap(char *filename)
{
  ...
  if (DEBUG) { 
    fprintf(stderr, "readsnap(): Ngas = %ld\n", All.N_gas); 
    fprintf(stderr, "readsnap(): Nstar = %ld\n", All.N_star); 
    fprintf(stderr, "readsnap(): Ntotal = %ld\n", All.N_total); 
  ...
  allocate_star();
  allocate_bh();
  long Ngas=All.N_gas,Nhalo=All.N_halo,Ndisk=All.N_disk;
  long Nbulge=All.N_bulge,Nstar=All.N_star,Nbh=All.N_bh,Ntotal=All.N_total; 
  ...
  /* read in ID info */ 
  SKIP;
  if (Ngas > 0) for(n=0;n<Ngas;n++) fread(&PG[n].ID, sizeof(long), 1, fd);
  if (Nhalo > 0) for(n=0;n<Nhalo;n++) fread(&PH[n].ID, sizeof(long), 1, fd);
  if (Ndisk > 0) for(n=0;n<Ndisk;n++) fread(&PD[n].ID, sizeof(long), 1, fd);
  if (Nbulge > 0) for(n=0;n<Nbulge;n++) fread(&PB[n].ID, sizeof(long), 1, fd);
  if (Nstar > 0) for(n=0;n<Nstar;n++) fread(&PS[n].ID, sizeof(long), 1, fd);
  if (Nbh > 0) for(n=0;n<Nbh;n++) fread(&PBH[n].ID, sizeof(long), 1, fd);
  SKIP;
