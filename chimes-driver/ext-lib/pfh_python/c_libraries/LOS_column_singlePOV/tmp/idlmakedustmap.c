...
long getnh(long argc, void *argv[]) 
{
  ...
  long *oi, *ot; 
  float dummy[3];
  float *OUT_NH,*OUT_NH_HOT; 	/* output NH */ 
  float *OUT_Z;
  float theta, phi; 
  float nh_prefactor;
  float *on,*onh; 
  ...
  if((argc < 9) || (argc > 11)) {
    fprintf(stderr, "Expected 9-11 arguments (found %d)\n",argc); 
    exit(0); 
  }
  ...
  fprintf(stdout,"theta, phi = %.2f %.2f\n", theta, phi); 
...
*/
  ...
  //N_rays = (long)(sqrt(N_rays)+1) * (long)(sqrt(N_rays)+1);		/* Convert to a perfect square */
  printf("NUMBER_OF_RAYS = %d, N_rays = %d, sizeof(Ray_struct) = %ld\n", NUMBER_OF_RAYS, N_rays, sizeof(Ray_struct) ); 
  ...
  if(!(ray = malloc(N_rays*sizeof(struct Ray_s))))
     {
        printf("failed to allocate memory: N_rays= %d  size= %ld\n",N_rays,N_rays*sizeof(struct Ray_s));
        exit(0);
     }
 ...
 */
  ...
  //printf("t1\n");fflush(stdout);
    ...
    //printf("t1find\n");fflush(stdout); 
    //printf("to find: num %d px py pz %g %g %g \n",ray_num,ray[ray_num].pos[0],ray[ray_num].pos[1],ray[ray_num].pos[2]);fflush(stdout);
#ifndef DIRECT_RAY_INTEGRATION
    ...
    // if (DEBUG) 
    if (!(ray_num%10000)) printf("ray_num %d origin_cell %d origin_type %d origin_id %d theta %.1f phi %.1f nh %.3f Z %f\n",ray_num, RAY_ORIGIN_CELL, ray[ray_num].ORIGIN_TYPE, ray[ray_num].ORIGIN_ID,ray[ray_num].theta,ray[ray_num].phi,log10(ray[ray_num].nh_hot),ray[ray_num].Z);fflush(stdout);
