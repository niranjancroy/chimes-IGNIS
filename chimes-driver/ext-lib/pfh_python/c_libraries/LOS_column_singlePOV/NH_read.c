...
long main()
{
	...
	long ray_origin_type;
	long number_of_ray_origins;
	long number_of_rays_per_origin;

    fread(&simulation_time,sizeof(float),1,output_file);
    fread(&ray_origin_type,sizeof(long),1,output_file);
    fread(&number_of_ray_origins,sizeof(long),1,output_file);
    fread(&number_of_rays_per_origin,sizeof(long),1,output_file);
	printf("%i %i %i %f \n",ray_origin_type,number_of_ray_origins,number_of_rays_per_origin,simulation_time);
    
    long nO = number_of_ray_origins;
    long nR = number_of_rays_per_origin;
    long i=0,j=0;
    float *theta_list;
	...
	for (i=0; i<nO; i++) {
	    fread(&ray_origins[i].ray_source_ID,sizeof(long),1,output_file);
