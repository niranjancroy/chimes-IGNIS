...
#define TREE_FRAC_HSML        0.33  /* Fraction of h_sml for minimum size of tree cells */
#define FRACTIONAL_STEPSIZE   0.25	/* Fraction of h_sml to step out when using brute-force integration */
...
struct global_data_all_processes
{
  ...
  /* some SPH parameters */
  long DesNumNgb;                /*!< Desired number of SPH neighbours */
  long MaxNumNgbDeviation;       /*!< Maximum allowed deviation neighbour number */
  ...
  long ComovingIntegrationOn;	/*!< flags that comoving integration is enabled */
  long PeriodicBoundariesOn;     /*!< flags that periodic boundaries are enabled */
  long ResubmitOn;               /*!< flags that automatic resubmission of job to queue system is enabled */
  long TypeOfOpeningCriterion;   /*!< determines tree cell-opening criterion: 0 for Barnes-Hut, 1 for relative
                                  criterion */
  long TypeOfTimestepCriterion;  /*!< gives type of timestep criterion (only 0 supported right now - unlike
                                  gadget-1.1) */
  long OutputListOn;             /*!< flags that output times are listed in a specified file */
  long CoolingOn;                /*!< flags that cooling is enabled */
  long StarformationOn;          /*!< flags that star formation is enabled */
...
typedef struct LOCALVALS_s{
	long Ngbs;					/* store the number of nearest neighbors */
...
//  long ORIGIN_TYPE;			/* particle type of the particle from which the ray originates */
//  long ORIGIN_ID;			/* ID (particle number) for the particle which is the ray origin */
  ...
  long N_steps;				/* number of distance steps to the edge of the box */
...
struct CELL_STRUCT{
  ...
  long sub_cell_IDs[N_SUBCELL_3];/* ID numbers of the sub-cells contained (if present) */
  ...
  long *particle_IDs;			/* vector of particle indices of the particles contained in the cell */
...
struct snap_header
{
  long      npart[6];		// Number of particles of each type
  ...
  long      flag_sfr;		// Flag - if star formation turned on and info. kept
  long      flag_feedback;	// Flag - if feedback of star formation turned on
  long      npartTotal[6];	// Total number of particles of each type (should be identical to npart)
  long      flag_cooling;	// Flag - if cooling physics turned on
  long      num_files;		// Indicates if snapshot split among several files (for large runs)
