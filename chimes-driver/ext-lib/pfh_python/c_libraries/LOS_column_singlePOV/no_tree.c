...
void line_of_sight(Ray_struct *ray)
{
	...
	long n_steps, n_steps_max,r_i;
	LOCALVAL local_temp;
	//local_temp = (LOCALVAL *)malloc(sizeof(LOCALVAL));
	
	x_max = 1.0*(float)MAX_DISTANCE_FROM0;	
	x_min = -x_max;
	n_steps_max = (long)LINE_NMAX;
	dr_min = fabs(2.0*x_max) / (2000.0 * (float)n_steps_max);
...
void local_vars(float x, float y, float z, LOCALVAL *vals)
{
  ...
  float r0[3];					/* Position vector (from input) */
  	...
  	r0[1] = y;
  	r0[2] = z;
  long particle_id = 0;			/* Identifies which type of particles we're going
...
{
  long Num_neighbors = (long)NUMBER_OF_NEIGHBORS;	
  long n_steps_max = (long)LINE_NMAX;
