...
//#include <sys/time.h>                // for gettimeofday; one option of measuring time left
    ...
    float* OUT0, float* OUT1, float* OUT2, float* OUT3 ) // output vectors with final weights
{
  ...
  long n,i,j,k,imin,imax,jmin,jmax,N_KERNEL_TABLE,print_modulus;
...
//  struct timeval t1, t2;
  ...
  // start timer if using sys/time.h
//  gettimeofday(&t1, NULL);
  print_modulus = N_xy / 20;   // print 20 times in the loop

  // get the current time with time.h
  time(&t1);
  ...
  // that's a task for another day though
  // for now, it's much faster to parallelize the movie making by having each processor do a different chunk of snapshots
  for(n=0;n<N_xy;n++)
  {
    ...
    // now loop over pixels that this particle (once smeared by the kernel) falls on and do the sum
    for(i=imin;i<imax;i++)
    {
     dx_n = x[n]-x_i[i]; 
     if (fabs(dx_n) < h)
     ...
     for(j=jmin;j<jmax;j++)
     {
       ...
       if (r2_n < h2) 
       {
        ...
        if(wt1[n] != 0.) OUT1[k] += wt1[n]*wk; // adds 'surface brightness' of wt1 to total in channel1
        if(wt2[n] != 0.) OUT2[k] += wt2[n]*wk;
        if(wt3[n] != 0.) OUT3[k] += wt3[n]*wk;
       } // if (r2_n < h2)
      } // for(j=jmin;j<jmax;j++)
      } // if (x2_n < h2)
    } // for(i=imin;i<imax;i++)
    ...
    {
      pct_done = n*100./N_xy;
      printf("\t%g%%, %ld..  xy=%g|%g  mass=%g wt(1|2|3)=%g|%g|%g  h=%g  imin/max=%ld|%ld  jmin/max=%ld|%ld  \n",
          pct_done,n,x[n],y[n],Mass[n],wt1[n],wt2[n],wt3[n],h,imin,imax,jmin,jmax); 

     if(n>0)
        {
    //      // get time elapsed with sys/time.h
    //      gettimeofday(&t2, NULL);
    //      // compute the elapsed time in minutes with sys/time.h
    //      elapsedTime = (t2.tv_sec - t1.tv_sec) / 60.0;

          // instead, do it with time.h
          time(&t2);

          //elapsed time in minutes:
          elapsedTime = difftime(t2, t1) / 60.;    //difftime returns first minus second

          //what time is it now in human readble (only need HMS)?
    //      time_string = ctime(t2);
          strftime(time_string, sizeof(time_string), "%H:%M:%S", localtime(&t2) );

          // ok, so if it's taken elapsed time to do x% of the calculation, then scaling linearly:
          total_est = 100./pct_done * elapsedTime;

          if(total_est - elapsedTime < 3) printf("\testimate %g seconds left at %s\n", 60*(total_est - elapsedTime), time_string);
          else  printf("\testimate %g minutes left at %s\n", total_est - elapsedTime, time_string);
        }
      fflush(stdout); // end time print statement
    } // end print statement
  ...
  time(&t2);
  elapsedTime = difftime(t2, t1) / 60.;    //difftime returns first minus second
  printf("ray-trace took a total of %g minutes\n", elapsedTime);
  fflush(stdout);
