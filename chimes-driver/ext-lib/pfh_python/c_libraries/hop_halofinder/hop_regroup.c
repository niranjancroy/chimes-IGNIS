...
//	    printf("gd1 %g gd2 %g dens %g pd %g sd %g d0 %g \n",
//	        gdensity[g1],gdensity[g2],dens,peakdensthresh,saddledensthresh,densthresh);
	    fflush(stdout);
	...
	if (gdensity[g1]>=peakdensthresh && gdensity[g2]>=peakdensthresh)
	{
	    /* these are all PFH additions for better separation of stellar-particle halos */
	    //continue; /* no merging because of boundary density criterion */
/*
	    float frac_saddle_thold_major = 0.99999; //1./2.; // max density relative to primary (more dense) system of saddle
	    float frac_saddle_thold_minor = 0.99999; //1./1.1; // max density relative to secondary (less dense) system
	    if (gdensity[g1]>gdensity[g2]) 
	        if((dens < gdensity[g1]*frac_saddle_thold_major) && (dens < gdensity[g2]*frac_saddle_thold_minor))
	            continue;
	    else
	        if((dens < gdensity[g2]*frac_saddle_thold_major) && (dens < gdensity[g1]*frac_saddle_thold_minor))
	            continue;
	    //if ((dens<gdensity[g1]/2.) && (dens<gdensity[g2]/2.)) continue;
*/
	    /* end PFH additions */
