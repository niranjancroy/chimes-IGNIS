...
def cross_section( f0, METALLICITY_OVER_SOLAR=1.0 ):
    ...
    SMC_key=1; LMC_key=0; MW_key=0;
    if (SIGMA.size > 1):
        ok=(f0 < f_003keV)
        lambda_microns = 2.998e14 / f0[ok];   # convert frequency [Hz] to wavelength [microns]
        xsi = pei_dustparam( lambda_microns, SMC=SMC_key, LMC=LMC_key, MW=MW_key );
...
def opacity_per_solar_metallicity( f0 ):
    mu = 1.42 ## assume dense gas (atomic) - calibration from Draine et al.
    albedo = 0.6 ## also calibrated from Draine et al.
    R_V = 5.5 ## depends on dust assumptions and opacity model: 
    return cross_section( f0, METALLICITY_OVER_SOLAR=1.0 ) / ( mu * 1.67e-24 ) * (1.-albedo) * (3.1/R_V); 
