...
spec = [
    ...
    ('Luminosities', float64[:]), # total mass of node
...
class DynamicOctree(object):
    ...
    def __init__(self, points, luminosities, softening, vels, morton_order=True, quadrupole=False):
        self.TreewalkIndices = -ones(points.shape[0],dtype=np.int64)        
        self.HasQuads = quadrupole
        children = self.BuildTree(points, luminosities, softening, vels) # first provisional treebuild to get the ordering right
        ...
        if morton_order: # if enabled, we rebuild the tree in Morton order (the order that points are visited in the depth-first traversal)
            children = self.BuildTree(points[self.TreewalkIndices], np.take(luminosities,self.TreewalkIndices), np.take(softening,self.TreewalkIndices), vels[self.TreewalkIndices]) # now re-build the tree with everything in order
            SetupTreewalk(self,self.NumParticles,children) # re-do the treewalk order with the new indices

        ComputeMomentsDynamic(self,self.NumParticles,children) # compute centers of mass, etc.
            
    def BuildTree(self,points,luminosities,softening,vels):
        ...
        self.Luminosities = zeros(self.NumNodes)
        if self.HasQuads:
...
#        self.ParentNode = -ones(self.NumNodes, dtype=np.int64)
        ...
        self.Luminosities[:self.NumParticles] = luminosities
...
@njit
def ComputeMomentsDynamic(tree, no, children): # does a recursive pass through the tree and computes centers of mass, total mass, max softening, and distance between geometric center and COM
    quad = zeros((3,3))
    if no < tree.NumParticles: # if this is a particle, just return the properties
        return tree.Softenings[no], tree.Luminosities[no], quad, tree.Coordinates[no], tree.Velocities[no], 0
    else:
        m = 0
        com = zeros(3)
        vel = zeros(3)
...
#        vdisp = vdisp/m
        for c in children[no]:
            if c > -1:
                ...
                vdisp += tree.Luminosities[c] * (dv[0]*dv[0] + dv[1]*dv[1] + dv[2]*dv[2])
        vdisp = vdisp/m
        if tree.HasQuads:
            for c in children[no]:
                if c > -1:
