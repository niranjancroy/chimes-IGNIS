...
def find_haloids(simdir,snapnum,theIDs):
   ...
   AHFile = glob( simdir + '/AHF/snap' + g.snap_ext(snapnum) + 'Rpep..z*.AHF_particles')[0]
...
def read_AHF_halos( simdir, snapnum, AHFdir='AHF', readheader=True, h=h ):
   ...
   if len( glob(simdir + '/' + AHFdir + '/snap' + g.snap_ext(snapnum) + '*.AHF_halos') ) == 0:
     ...
     if len( glob( simdir + '/' + AHFdir + '*.AHF_halos') ) == 0:
       print '....AHF data not found....'
       return {'k':-1}
...
def read_AHF_halos_paco( simdir, AHFdir, snapnum ):

   glob_str = AHFdir + '/snap' + g.snap_ext(snapnum) + '*.AHF_halos'

   if len(glob(glob_str)) == 0:
     print '....AHF data not found....', glob_str
     return {'k':-1}
   else:
     AHFile = glob(glob_str)[0]
   ...
   hfile = open( AHFile )
   htxt_ids = np.loadtxt(hfile, usecols=(0,1), dtype=np.uint64 )
   hfile.close()

   header = g.readsnap(simdir,snapnum,0,cosmological=1,skip_bh=1,header_only=1,snapshot_name='snap')
   if header['k'] == -1:
     print 'header not found!', simdir, snapnum, snapshot_name
     return {'k':-1}
   redshift = header['redshift']
   h = header['hubble']
   print snapnum, '  z = ', AHFile[-16:-10].split('z')[-1], header['redshift'], '  h =', h
   ...
   halos = { 'k': 1,
             'z': redshift,
             #'id': htxt[:,0].astype('uint64'),
             #'HostID': htxt[:,1].astype('uint64'),
             'id': htxt_ids[:,0],
             'HostID': htxt_ids[:,1],
             'Mvir': htxt[:,3] * hinv,     # in Msun
             'Mgas': htxt[:,47] * hinv,
...
#   13-15: center, 16-18: CM velocity, 19-21: pos. of most bound particle.
     ...
     if old == 0:
       data = { 'GalID': ftxt[:,0].astype(int),
                ...
                'VcMax': ftxt[:,5] * ascale * unit_Velocity / CM_PER_KM,                         # in km/s (physical)  --> WRONG: it should be /= sqrt(ascale) !!!
...
def read_blackhole_simba( simdir, redo=0 ):

   outfile = simdir + '/blackhole_details/bhALL.hdf5'

   names = [ 'a', 'ID', 'Mass', 'BH_Mass', 'Mass_AlphaDisk', 'Mdot', 'Mdot_bondi', 'Mdot_alphadisk', 'dt',
             'rho', 'e', 'Sfr', 'Mgas', 'Mgas_hot', 'Mstar', 'MgasBulge', 'MstarBulge', 'R0', 'p', 'v', 'Jgas', 'Jstar' ]
   formats = [ 'f8', 'i8', 'f8', 'f8', 'f8', 'f8', 'f8', 'f8', 'f8',
               'f8', 'f8', 'f8', 'f8', 'f8', 'f8', 'f8', 'f8', 'f8', '(3,)f8', '(3,)f8', '(3,)f8', '(3,)f8' ]
   ...
   if (os.path.isfile(outfile)) and (redo == 0):
      ...
      for ns in range(600):
         try:
           header = g.readsnap(simdir,ns,0,cosmological=1,skip_bh=1,header_only=1)
           break
         except Exception:
           print simdir + '  ...snapshot not found:', ns
      ...
      bh['Mgas_hot'] *= ( UnitMass_in_Msun / h )
      ...
      bh['Mdot_bondi'] *= UnitMdot_in_Msun_per_yr
...
def read_blackhole( simdir, redo=0, old=0 ):
   ...
   if (os.path.isfile(outfile)) and (redo == 0):
      ...
      #except Exception:
      #  header = g.readsnap(simdir,1,0,cosmological=1,skip_bh=1,header_only=1)
...
def read_blackhole_winds( simdir, redo=0 ):
   ...
   if (os.path.isfile(outfile)) and (redo == 0):
      ...
      if j > 0:
        ...
        ascale = bh['a'][:]                               # a is the scale factor
                                                          # ID is the gas particle ID

        bh['Mass'] *= ( UnitMass_in_Msun / h )            # gas particle mass in Msun

        for k in range(3):
         bh['p'][:,k] *= ( ascale / h )                 # gas particle position in kpc (physical)
         bh['v'][:,k] /= ascale                         # gas particle velocity (after kick) in Km/s (physical)                   
         bh['p_bh'][:,k] *= ( ascale / h )              # BH position in kpc (physical)
                                                        # d is the unit vector in the direction of the velocity kick
                                                        # ID_bh is the ID of the BH particle
...
def blackhole_track( id, bhALL, bhMRG=-1, bhBAL=-1 ):
   ...
   if (util.checklen(bhMRG) > 1) and (util.checklen(bhBAL) > 1):
     return thebh, thebh_mrg, thebh_bal
   elif (util.checklen(bhMRG) > 1):
     return thebh, thebh_mrg
   else:
     return thebh
...
def bh_accretion( bhstruct, Mbh_ini, eff, EddLimit=1000, mode='torque' ):
   ...
   for i in range(istart,0,-1):
      ...
      R0 = bhstruct['R0'][i]
      GasMass = bhstruct['Mgas'][i]
      StarMass = bhstruct['Mstar'][i]
      MbulgeGas = bhstruct['MgasBulge'][i]
      MbulgeStar = bhstruct['MstarBulge'][i]
      SFR = bhstruct['Sfr'][i]

      if mode == 'torque':
        alphaK = 5.
        Mtot = GasMass + StarMass
        Md = Mtot - MbulgeStar                # all gas assigned to the disk!
        if (GasMass > 0) & (Md > 0) & (R0 > 0):
          fgas = GasMass / Md
          fd = Md / (GasMass + StarMass)
          f0 = 0.31 * fd**2. * (Md/1e9)**(-1./3.)
          MdotTorque = alphaK * fd**(5./2.) * (Mbh[i]/1e8)**(1./6.) * (Mtot/1e9) * (R0*1000./1e2)**(-3./2.) * (1 + f0/fgas)**(-1)
          Mdot[i] = eff * MdotTorque
        ...
        if (GasMass > 0):
          Mtot = GasMass + StarMass + Mbh[i]
          Tdyn = np.sqrt( (R0*CM_PER_KPC)**3 / (G_UNIV*Mtot*MSUN) ) / SEC_PER_YEAR
          Mdot[i] = eff * GasMass / Tdyn

      EddingtonLimit = MdotEddington(Mbh[i])
      if Mdot[i] > EddLimit*EddingtonLimit:
        Mdot[i] = EddLimit*EddingtonLimit
      Mbh[i-1] = Mbh[i] + Mdot[i] * dt[i-1]
...
def evaluate_torque( thebh, eff ):

   nstep = thebh['a'].size

   MdotTorque = np.zeros(nstep)
   f0 = np.zeros(nstep)

   Mbh = thebh['BH_Mass']
   R0 = thebh['R0']
   GasMass = thebh['Mgas']
   StarMass = thebh['Mstar']
   MbulgeGas = thebh['MgasBulge']
   MbulgeStar = thebh['MstarBulge']

   ind_bulge = np.where( MbulgeStar - StarMass > 0 )[0]
   if ind_bulge.size > 0:
     MbulgeStar[ind_bulge] = StarMass
   ...
   # --- accretion rate per unit Mbh**(1/6)  [Msun/year]
   MdotTorque[ind] = eff * alphaK * fd[ind]**(5./2.) * (Mbh[ind]/1e8)**(1./6.) * (Md[ind]/1e9) * (R0[ind]*1000./1e2)**(-3./2.) * (1 + f0[ind]/fgas[ind])**(-1)
...
def file_list( path, ext='' ):
   full_path = glob( path + '*' + ext )
   nfile = len(full_path)
   filelist = np.zeros(nfile,dtype='int')
   filestr = []
   for i in range(nfile):
      numstr = full_path[i][ full_path[i].find('_',-10,-1)+1 : len(full_path[i])-len(ext) ]
      filestr.append(numstr)
      filelist[i] = int( numstr )
   filestr.sort()
   filelist.sort()
   return filelist, filestr
...
def NEAREST_XYZ(dr,BoxSize):
   # dr = dr[ npart, ndim ]
   # x=((x)>boxHalf_X) ? ((x)-boxSize_X)  :  (((x)<-boxHalf_X) ? ((x)+boxSize_X):(x))
   BoxHalf = BoxSize / 2.
   for k in range(3):
      ind = np.where( dr[:,k] > BoxHalf )[0]
      if ind.size > 0:
        dr[ind,k] = dr[ind,k] - BoxSize
      ind = np.where( dr[:,k] < -1.*BoxHalf )[0]
      if ind.size > 0:
        dr[ind,k] = dr[ind,k] + BoxSize
   return dr
