# >>> execfile('bhfeedback_windzoomplot_movie_submit.py')
...
for ICdir in IC_list:
 for sname in sim_list:
  ...
  for n, i in enumerate(range(0,nsnap,nsnap_per_task)):
     ...
     py_str = 'python bhfeedback_windzoomplot_movie.py  ' + basedir + '  ' + run_name + '  ' + str(i_ini) + '  ' + str(i_end) + '  > log/' + log_name + '.' + str(n) + '.log\n\n'
     print py_str
     sh_name =  'tmp.sh'
     f = open(sh_name,'w')
