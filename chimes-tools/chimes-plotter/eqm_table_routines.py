...
def plot_eqm_table(plotter_pars):    
    ...
    ## List storing the abundances of individual
    ## species as separate arrays
    abundance_array_list = []

    ## The dimensions of these arrays need to be
    ...
    for chimes_species in plotter_pars["species_list"]:
        ...
        species_array = np.transpose(h5file["Abundances"][:, :, :, chimes_index], transpose_order) 

        abundance_array_list.append(10.0 ** species_array)
        ...
        for j in range(subplot_grid_cols):
            for k in range(N_species):
                for l in range(plotter_pars["N_lines"]): 
                    ...
                    try:
                        try: 
                            ax[i, j].plot(x_var, np.log10(abundance_array_list[k][:, line_var_indices[l], panel_var_indices[panel_index]]), linestyle = line_styles[l], color = matplotlib.cm.viridis(col_index), linewidth = plotter_pars["line_width"])
                        except IndexError:
                            ax[j].plot(x_var, np.log10(abundance_array_list[k][:, line_var_indices[l], panel_var_indices[panel_index]]), linestyle = line_styles[l], color = matplotlib.cm.viridis(col_index), linewidth = plotter_pars["line_width"])
                            
                    except TypeError: 
                        ax.plot(x_var, np.log10(abundance_array_list[k][:, line_var_indices[l], panel_var_indices[panel_index]]), linestyle = line_styles[l], color = matplotlib.cm.viridis(col_index), linewidth = plotter_pars["line_width"])
    ...
    y_label = r"$\log_{10} [n_{\rm{i}} / n_{\rm{element}}]$" 
